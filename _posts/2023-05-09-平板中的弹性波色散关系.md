...
$$
u_x = i \bigg( Bk\cos\alpha z + C\beta\cos\beta z \bigg) e^{i(\xi x-\omega t)}
$$

$$
u_z = \bigg(-B\alpha\sin\alpha z + Ck\sin\beta z\bigg) e^{i(\xi x-\omega t)}
$$

此时 $\tau_{zz}$ 和 $\tau_{xz}$ 的边界条件为：

$$
\bigg(\xi^2-\beta^2\bigg)B\cos\alpha b + 2\xi\beta C\cos\beta b = 0
$$

$$
\pm i \bigg[ -2\alpha \xi B\sin\alpha b + \bigg(\xi^2-\beta^2\bigg)C\sin\beta b \bigg] = 0
$$

得到对称波在平板中传播的 Rayleigh-Lamb 频率方程：

$$
\frac{\tan\beta b}{\tan\alpha b} = -\frac{4\alpha\beta \xi^2}{\bigg(\xi^2-\beta^2\bigg)^2}
$$
...
$$
F(\alpha,\beta,\xi) = \frac{\tan\beta b}{\tan\alpha b} + \bigg\{\frac{4\alpha\beta \xi^2}{\bigg(\xi^2-\beta^2\bigg)^2}\bigg\}^{\pm 1} = 0
$$
...
$$
\frac{\tan\beta' b}{\tan\alpha' b} = -\bigg\{\frac{4\alpha'\beta' \xi^2}{\bigg(\xi^2-\beta'^2\bigg)^2}\bigg\}^{\pm 1}
$$
