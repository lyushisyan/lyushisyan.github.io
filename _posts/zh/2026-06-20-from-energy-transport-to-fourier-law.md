---
layout: post
title: "从微观能量输运到傅里叶定律：它何时成立，何时失效？"
lang: zh
...
reading_time: "22 min"
description: "从微观载流子输运出发，理解傅里叶定律如何在长时间、长长度尺度下出现，以及扩散、弹道、流体动力学、瞬态和相干热输运之间的物理区别。"
...
## 7. 声子流体动力学：不是普通扩散，也不是简单弹道

声子散射过程对晶格动量的作用并不相同。

Normal 过程满足晶格动量在第一布里渊区内守恒。它可以快速重新分配声子之间的能量和动量，却不会直接破坏声子系统的总准动量。

阻性过程则会使热流相关动量弛豫，包括 Umklapp 散射、同位素散射、缺陷散射，以及某些具有动量耗散作用的边界散射。

如果 Normal 散射的特征时间远短于阻性散射时间，

$$
\tau_N\ll\tau_R,
$$

声子可能在总动量被破坏之前，首先形成带有集体漂移速度的局域准平衡分布。

在有限尺寸系统中，一个典型的流体动力学尺度窗口可示意为

$$
\Lambda_N\ll L\ll\Lambda_R,
$$

其中 $\Lambda_N$ 和 $\Lambda_R$ 分别对应 Normal 和阻性散射的特征长度。

第一个不等式保证声子之间能够建立集体局域平衡，第二个不等式保证这种集体动量在穿越系统时不会立即被阻性散射破坏。

此时，热输运不再是相互独立声子的简单随机游走，而可能表现出：
...
其中 $\ell$ 是与动量扩散相关的非局域长度，具体系数取决于材料对称性和微观碰撞算符。

与普通傅里叶定律相比，这里的空间导数项表明：某一点的热流不仅由该点的温度梯度决定，还受到周围热流分布的影响。

声子流体动力学也说明，仅仅比较总 Normal 散射率和总 Umklapp 散射率通常是不充分的。是否形成流体动力学输运，还取决于散射过程如何耦合不同声子模式、动量如何在模式空间中演化，以及边界能否维持或破坏集体漂移。
...
传统 BTE 主要追踪各个声子模式的布居 $n_\lambda$，相当于只保留振动密度矩阵的对角部分。

当不同振动模式的频率间隔明显大于其线宽时，各模式可以被清楚地区分，声子布居图像通常非常有效。

然而，在复杂晶体、强非简谐材料和无序固体中，近简并模式的频率间隔可能与其线宽相当：
...
此时，不同模式之间的非对角相干可能在能量输运中发挥作用。仅仅追踪每个模式的独立布居，就可能遗漏模式间的相干能量传递。

Wigner 输运或密度矩阵方法可以同时描述：

* 类似声子气体的布居传播；
* 近简并振动态之间的相干耦合；
* 从有序晶体到无序固体之间的连续过渡。

需要强调的是，微观上存在相干贡献，并不意味着宏观上一定没有傅里叶定律。经过足够长的时间和空间粗粒化后，系统仍可能表现为

$$
\boldsymbol q=-\boldsymbol\kappa\nabla T.
$$

区别在于，此时 $\boldsymbol\kappa$ 的微观来源不能再由仅包含独立声子布居的动理学公式完整描述。

## 10. 不同输运区间并不是同一条轴上的简单排序
